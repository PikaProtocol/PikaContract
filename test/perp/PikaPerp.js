...
describe("PikaPerp", function () {
  ...
  beforeEach(async function () {
    ...
    this.uri = "URI"
    this.coeff = "36000000000000000000000000000000000000000000000" // 5e46, 5e10 * 1e18 * 1e18
    this.reserve = "10460634124736731369760000" // 1e25, 10e7 * 1e18, representing $10m usd
  ...
  //   it("should test okay", async function() {
  //     const size = "33542588015819136000000"
  //     const minGet = "10563697639884298000"
  //     const strike = await this.pikaPerp.getStrikeFromLeverage("1000000000000000000", true)
  //     const referrer = this.referrer.address
  //     // await expect(this.pikaPerp.openLongTest(size, strike, minGet, referrer, { from: this.alice.address, value: "1000000000000000000", gasPrice: "0" })).to.be.revertedWith("min get constraint violation")
  //     const tx = await this.pikaPerp.openLongTest(size, strike, minGet, referrer, { from: this.alice.address, value: "1000000000000000000", gasPrice: "0" })
  //     console.log('transaction', tx);
  //   })
  // })
    it("should openLong success", async function () {
      const size = "52447525809909224000000" // 1000 usd
      // const minGet = "300000000000000000" // 0.3 eth
      const strike = await this.pikaPerp.getStrikeFromLeverage("1000000000000000000", true)  // the strike for 5x long is 1667
      console.log('strike', strike.toNumber());
      const referrer = this.referrer.address

      const latestMark = await this.pikaPerp.getLatestMark()
      console.log('latest mark', latestMark.toNumber())
      const spotPx = await this.pikaPerp.getSpotPx()
      console.log('spot px', spotPx.toNumber())
      const coeffValue = await this.pikaPerp.coeff()
      console.log('coeff', coeffValue.toString())
      // const shift = await this.pikaPerp.shift()
      const shift = '-11883137162375';
      console.log('shift', shift.toString())
      const reserve = await this.pikaPerp.reserve()
      console.log('reserve', reserve.toString())
      const tradingFee = await this.pikaPerp.tradingFee()
      console.log('tradingFee', tradingFee.toString())

      const nextReserve = parseInt(reserve) + parseInt(size);
      const base = parseInt(coeffValue) / parseInt(reserve) - parseInt(coeffValue) / nextReserve;
      const premium = parseInt(shift) * nextReserve / 1e18 - parseInt(shift) * parseInt(reserve) / 1e18;
      const sellAmount = base + premium

      // const initSize = (parseInt(size) * (1 - tradingFee / 1e18)).toString()
      // const initSize = size
      const initSize = (parseInt(size) * ( (-1) * parseInt(shift) / parseInt(reserve)))
      // const nextReserve1 = parseInt(reserve) + parseInt(size)
      const nextReserve1 = parseInt(reserve) + parseInt(initSize)
      const base1 = parseInt(coeffValue) / parseInt(reserve) - parseInt(coeffValue) / nextReserve1;
      const premium1 = parseInt(shift) * nextReserve1 / 1e18 - parseInt(shift) * parseInt(reserve) / 1e18;
      console.log('reserve, next reserve', reserve.toString(), nextReserve1.toString())
      console.log('base 1, premium1', base1, premium1)
      const sellAmount1 = base1 + premium1
      console.log('sell amount', sellAmount1);
      // const currentPrice = (1 - tradingFee / 1e18) * sellAmount1 / parseInt(size);
      const currentPrice = (1 - tradingFee / 1e18) * sellAmount1 / parseInt(initSize)
      const entryPrice = (1 - 0.005) * currentPrice
      const slip = (currentPrice / entryPrice - 1) * 100
      console.log('slippage', slip)
      const minGet1 = entryPrice * parseInt(size);
      console.log('minget amount', minGet1.toString())


      console.log('sell amount from code expectation', sellAmount.toString())

      // const initialEthBalance = await provider.getBalance(this.alice.address)
      await this.pikaPerp.openLong(size, strike, minGet1.toString(), referrer, {from: this.alice.address, value: "52447525809909224000000", gasPrice: "0"}) // 1eth
      // // verify eth paid
